...
| **P3 -- score** | history and state, scoring, ranking, categories. D4 decided | the seven invariants enforced by property tests; bootstrap-from-nothing passes | not started |
...
- [x] Test: each of the seven scoring invariants -- [T-043](../TODO/scoring.md) and [T-045](../TODO/scoring.md), done. Executable properties over any candidate rather than over a model, because none is chosen (T-044). ⭐ The obvious candidate is **refuted**: a plain success rate fails I2, since 1 of 1 and 500 of 500 both score 1.0. A Wilson lower bound passes all seven. ⛔ **I7 is T-045's prohibition made checkable** and it rejects a candidate somebody would plausibly write: "only trackers that have never failed" lets one failure decide the order at any sample size
...
- [x] The politeness budget is computed, published, and asserted by a test against the configured schedule -- [T-026](../TODO/measurement.md). `src/trackers/politeness.py` computes it from the records a run produced, `render_sweep` publishes it, and the test reads `.github/workflows/health-sweep.yml` rather than a constant. A full-corpus sweep costs **6072 DNS lookups at worst against a ceiling of 100,000**, over the 759 name-addressed hosts of 965 -- an address literal costs no lookup -- and 10,616 probes a day at D7's cadence. ⚠ The per-day figures are a projection: the sweep has no schedule trigger, and the workflow architecture that would add one is a separate open entry
...
- [x] Test: history housekeeping preserves the dataset and never touches `main` -- [T-081](../TODO/operations.md), done. ⭐ The threshold is **derived**: `experiments/34` measures 14.3 KiB per commit, so a full clone stays under 250 MiB for 17,853 commits, about 6.1 years at eight publishes a day -- against an inherited 5000, which would be 1.7 years here. ⛔ The count is a proxy whose validity expires, because `state.jsonl` grows and T-042 projects it at 23.4 MB in five years, so the measured size decides
...
- [x] `latest` / `daily` / `weekly` semantics defined, implemented, and verified against real platform behaviour -- [T-064](../TODO/publication.md). **The platform half is now measured** ([T-003](../TODO/claims.md), closed): `/releases/latest` ignores both a tag named `latest` and a newer prerelease, an asset can be replaced at a stable URL but **not** read back immediately, and moving a tag does **not** move the release, so delete-and-recreate is the route. **Defined and implemented 2026-09-08** in `src/trackers/channels.py`: the rolling channel is not called `latest`, a channel is updated by delete-and-recreate, a period is published once, the week is ISO, and the module imports no network module at all so it cannot verify a publish by reading a URL back. `tests/test_channels.py` reads `experiments/24`'s committed result and fails if a design choice rests on something that run refuted. ⚠ Nothing publishes yet, which is the data-branch entry
...
- [x] Security review complete; no upstream content is ever executed or interpolated into a shell command -- structurally held (no shell layer); review is [T-086](../TODO/operations.md), done. `tests/test_acquisition_security.py`, one class per threat, and the review `HISTORY/reviews/2026-09-09-12-acquisition-path.md`. ⭐ Two of the four are **unreachable rather than mitigated**: there is no shell call in `src/` at all, and nothing decompresses, so a bomb has no expansion step to exploit
