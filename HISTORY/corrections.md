...
| 9 | Row 3 above, again: the scheduled sweep rotates through the corpus, and each tracker is probed once per 21 hours | ⛔ **The rotation is the sampler and was never the ceiling.** `rotation_for` maps an instant to the three-hour bucket containing it, so two runs started inside one bucket take the **identical** slice and nothing refused the second. Runs `34281244142` (21:33:23Z) and `34289476724` (23:11:21Z) both reported `slice 5 of 7`, and **192 trackers carry observations 5878 s apart** in the published `state.jsonl` -- inside D7's 10800 s interval. D7 is read from the recorded history now, so no arithmetic over a clock can breach it | **Critical** -- RULES 4 is conduct toward other people's servers and this spent their requests twice; found by reading the published history rather than by any check, and the test named `..._share_no_tracker` compared two rotation integers |
| 10 | i2p and yggdrasil trackers are `unmeasurable`; `C-37` says both need routers this environment does not run | ⛔ **The sentence was about a vantage and was being read as a property of the trackers.** A router in a throwaway container costs one `podman run`, and with one: **3 of 11 i2p HTTP destinations answered**, and a yggdrasil node joined the overlay in **one second** and reached a peer. The category had been recorded unmeasurable in every sweep this project has ever taken, and the route that opened it was the one T-039 listed **last** and expected to cost the most | **High** -- not a wrong number but an unexamined constraint, which is worse: it closed 13 URLs to research on a premise nobody had tested |
| 11 | Addressing an i2p tracker by name measures the tracker | ⚠ **It measures our own addressbook first.** `opentracker.skank.i2p` timed out by name and answered **1677 bytes** when addressed by the b32 a public registry gave for it. 6 of 11 subjects are still name-unknown and are recorded as *our* naming failure rather than their silence | Medium -- it would have published three working destinations as unreachable |
| 12 | `.gitignore` covers the Windows reserved device names, so a stray `NUL` is handled | ⛔ **`check-citations.py` walks the working tree, not the index.** A `NUL` created by a redirect a shell did not map took the **whole gate** down with `ValueError: path is on mount '\\.\NUL'` -- a red gate naming no file anyone could act on. Both walkers skip reserved device names now, and the guard was proved by planting one and reading the exit code | Medium -- fails in the safe direction, and costs a Windows contributor an hour working out what the message means |

⚠ **Rows 3 and 9 are the same claim failing twice, for unrelated reasons**, and that is the finding rather than a coincidence. Both times the sentence *"the rotation covers the corpus"* was true of the selector and false of the runs: first because membership was positional and an upstream regenerating shifted it, then because the rotation's index came from a wall clock that scheduled runs do not honour. ⭐ **A property asserted about a mechanism is not a property of the system that uses it**, and the second failure was reachable only because the first fix left the ceiling resting on the mechanism.
