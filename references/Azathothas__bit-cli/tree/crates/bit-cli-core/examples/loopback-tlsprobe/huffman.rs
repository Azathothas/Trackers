...
//!
//! Needed because most clients Huffman-encode literal header *names* — without
//! this, any header outside HPACK's 61-entry static table decodes as an opaque
//! blob and the header-order capture is full of holes.

/// `(code, bit_length)` for symbols 0..=256. Index 256 is EOS.
#[rustfmt::skip]
...
/// Decode an HPACK Huffman string.
///
/// Returns `None` on a code that never resolves, on EOS appearing in the body
/// (a decoding error per RFC 7541 §5.2), or on padding longer than 7 bits.
pub fn decode(src: &[u8]) -> Option<String> {
    ...
    for byte in src {
        for bit in (0..8).rev() {
            ...
            // Codes are prefix-free, so the first (len, code) hit is the symbol.
            if let Some(sym) = TABLE.iter().position(|&(c, l)| l == len && c == code) {
                if sym == 256 {
                    return None; // EOS is not allowed inside the payload
                }
                out.push(sym as u8);
                code = 0;
                len = 0;
    ...
    // Whatever is left must be all-ones EOS padding, at most 7 bits.
