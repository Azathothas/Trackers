...
| [`check-vantage-metadata.py`](check-vantage-metadata.py) | a health record that does not say where it was measured from. ⚠ **Exits 2 until P2 exists, and that is correct**: returning 0 would report "every record carries its vantage" while checking nothing |
