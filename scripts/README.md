...
| [`update-state.py`](update-state.py) | folds a sweep's health records into the per-tracker history (T-040), and prints the shape of every history it wrote (T-041). ⛔ Never deletes a history, and **refuses to run against a corrupt state file** rather than starting a fresh one -- RULES 3.9, and it exits 1 leaving the file exactly as it was |
| [`source-quality.py`](source-quality.py) | what is **measured** about each source, beside the `Trust` a human **asserted** (T-101). ⛔ It reports and never acts: T-101's `Decision` is that both interesting findings run the opposite way from the obvious action -- a source with no unique entries may be the corroboration that makes another source's claim evidence, and one with many scruffy unique entries wants filtering rather than removal. Touches no third party: the uniqueness comes from the corpus and the rest from `sources.jsonl` |
| ⛔ [`raise-issues.py`](raise-issues.py) | the only script that writes to the issue tracker, and **dry run by default**: `--apply` is required to write anything. The decision is `src/trackers/issues.py` and is pure, so one issue per condition, closing when it clears, and a bounded body are unit-tested rather than integration-hoped. Deduplicates on a marker in the body, never on the title, and never touches an issue without one |
| ⛔ [`housekeep-data-branch.py`](housekeep-data-branch.py) | the only operation here that **discards commits**, and the hardest to fire by accident: `--apply` is required, the threshold must be crossed, a publication in flight refuses it, and it **never pushes** -- it prints the command. Safe because RULES 3.7 keeps history in tracked files, so the reset loses commits and not data, and it verifies every published file is still present before it will print anything. `main` is refused outright |
| ⛔ [`probe-corpus.py`](probe-corpus.py) | **the one script that contacts trackers.** BEP 34 is consulted per host first and no flag skips it; concurrency, per-host serialisation, timeout and deadline all bound it, and `ci` probes a sample. No offline mode, deliberately: its docstring says why. `--only-host` narrows a run to one hostname, and every report it writes carries what the run cost the people it measured (T-026) |
